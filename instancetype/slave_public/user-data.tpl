...
coreos:
  ...
  - name: mesos-slave.service
    ...
    content: |
      ...
      ExecStart=/usr/bin/docker run --privileged --net=host -e MESOS_DEFAULT_ROLE=${default_role} -e LIBPROCESS_IP=$private_ipv4 -e MESOS_HOSTNAME=${mesos_hostname} -e MESOS_IP=$private_ipv4 -e MESOS_MASTER=zk://leader.mesos:2181/mesos -v /sys/fs/cgroup:/sys/fs/cgroup -v /var/run/docker.sock:/var/run/docker.sock --rm=true --name mesos-slave -p 5050:5050 nixlike/mesos-slave:mesos-0.24.1_docker-1.10.0
